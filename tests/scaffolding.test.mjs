...
test('Scaffolding: tsconfig.json exists and enforces strict mode', () => {
  ...
  assert.equal(
    ...
    true,
    'tsconfig.json must have compilerOptions.strict set to true'
  );
...
test('Scaffolding: Next.js App Router basic structure exists', () => {
  assert.ok(
    fs.existsSync(path.join(rootDir, 'src', 'app', 'layout.tsx')),
    'src/app/layout.tsx must exist'
  );
  assert.ok(
    fs.existsSync(path.join(rootDir, 'src', 'app', 'page.tsx')),
    'src/app/page.tsx must exist'
  );
});
